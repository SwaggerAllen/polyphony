defmodule PolyphonyWeb.DebugDrawerLive do
  ...
  def render(assigns) do
    ...
    <div id="debug-drawer" class="fr stage dark dock">
      ...
      <%!-- A sliver, always. There used to be a full-width tab reading "⚙ log 12" and a
            control to tuck it down to this — but the count answered nothing you would
            open the log for, so tucked was the state it lived in and the rest was a step
            on the way there. The status dot is what remains: "is the socket alive" is
            the one thing worth reading without opening anything. --%>
      <Kit.btn kind={:ghost} type="button" id="debug-drawer-toggle" class="dock-tab rounded-full"
