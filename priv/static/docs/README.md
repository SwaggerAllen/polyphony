...
Four files and one tracker carry *why*, and they don't overlap:

- **As-built rationale → `architecture.md`.** Why a shipped part is the way it is.
- **Intended behavior → `behaviors/`.** What a screen should do, from the seat of the person using
  it. The overlap with `architecture.md` is real and the split is by *audience*: architecture answers
  "why is the code like this", behaviors answers "what is this screen supposed to do". A design
  thread reads the second and never the first.
...
2. **What a screen does from a user's seat — a state, a rule, a piece of copy?** → `behaviors/<screen>.md`,
   and bump its `rev`. That is the one doc a design thread writes into, and the only one that
   describes intent rather than as-built code.
3. **A neutral fact about what the backend can do today?** → `backend-capabilities.md`.
4. **A concrete task we've decided we want and know the shape of?** → a **Linear issue** in the
...
5. **The reasoning behind an unbuilt, post-v1 direction?** → `decisions.md`.
6. **Just when/what-order something ships?** → the issue's state and priority in Linear.
7. **A frontend visual/interaction decision?** → `ux/` (a mock or the UX README), not here.
8. **An issue you just finished?** → close it, and if it's worth remembering *how* it was built,
...
9. **Something we've decided *not* to build?** → the **Confirmed non-asks** document on the
